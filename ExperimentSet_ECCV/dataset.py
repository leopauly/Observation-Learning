...
def clusters_with_strides(x,num_clusters,cluster_length,height,width,channel,strides):
    clusters=np.empty([num_clusters,cluster_length,height,width,channel],dtype=np.uint8)
    print(clusters.shape)
    for i in range(num_clusters):
        w=0
...
def viewpoint_data(time_step,h,w,ch,imagefolderpath,gray,normalisation=True):
    ''' Reads video frames
    '''
    import scipy.misc as cv2 
    
    if (ch==1):
        X=np.zeros([time_step,h,w])
    else:
        X=np.zeros([time_step,h,w,ch])
    for i in range(time_step):
            img=cv2.imread(str(imagefolderpath+str(i)+'.png'))
            if (gray==True):
                img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            if(normalisation==True):
                img = img.astype('float32')
